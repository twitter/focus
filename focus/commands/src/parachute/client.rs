...
impl Client {
        ...
        I::IntoIter: 'a,
    {
        ...
        for source in sources {
            ...
            if let Some(build_dir) = self
                .find_closest_directory_with_build_file(&source_path, self.source.as_path())
    ...
    pub fn involved_sources(&self, coordinate: &str) -> Result<Vec<String>> {
        ...
        let output = Command::new("./bazel")
            ...
            .stdout(Stdio::from(
                File::create(&output_path).context("opening output for write")?,
            ))
            .spawn()
        ...
        let container: ActionGraphContainer;
        {
            ...
            let mut proto_output =
                File::open(&output_path).context("opening protobuf output for reading")?;
            proto_output
